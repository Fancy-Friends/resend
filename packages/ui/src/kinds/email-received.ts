...
/**
 * Email received — Start when an email arrives at one of your Resend receiving
 * addresses, with the email read in full.
 *
 * https://resend.com/docs/webhooks/emails/received
 *
 * Delivery: webhook. Point a receiving domain's MX record at Resend, then in
 * the Resend dashboard (Webhooks → Add webhook) point an endpoint at the route
 * your host mounts for this trigger and subscribe it to email.received. Put
 * the endpoint's signing secret -- shown as whsec_… -- on the connection as
 * `webhookSecret`, exactly as shown.
 */
...
export const RESEND_EMAIL_RECEIVED_TRIGGER_KIND = "@particle-academy/resend_email_received_trigger";
export const RESEND_EMAIL_RECEIVED_TRIGGER_OPERATION = "email_received";

export const RESEND_EMAIL_RECEIVED_TRIGGER_META = resendMeta("trigger", "an inbound email", "https://resend.com/docs/webhooks/emails/received");
 ...
 * and hoping.
 */
export const RESEND_EMAIL_RECEIVED_TRIGGER_OUTPUT: OutputField[] = [
  {
    "path": "type",
    "type": "string",
...
export const resendEmailReceivedTriggerKind: NodeKindDefinition = defineConnectorKind(RESEND_EMAIL_RECEIVED_TRIGGER_META, {
  name: RESEND_EMAIL_RECEIVED_TRIGGER_KIND,
  aliases: ["resend_email_received_trigger"],
  ...
  outputs: [{ id: "out" }],
  sideEffects: "none",
  outputShape: RESEND_EMAIL_RECEIVED_TRIGGER_OUTPUT,
  configSchema: [],
  defaultConfig: {
    "mode": "auto"
  },
  renderBody: ({ config }) =>
    summarize(RESEND_EMAIL_RECEIVED_TRIGGER_META, config as Record<string, unknown>, "an inbound email"),
});

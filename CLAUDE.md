...
npm run provider -- resend            # regenerate this repo
npm run provider -- resend --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/resend-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/resend-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/resend-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-resend` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/resend/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/resend` |
| npm | `@particle-academy/resend-ui`, `@particle-academy/resend-js` |
| Packagist | `particle-academy/resend-php` |
| PyPI | `fancy-resend` |
...
**Default to plain HTTP.** A Resend SDK is third-party code and is subject to
